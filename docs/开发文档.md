...
为了让登陆组件的背景色撑满，所以我们需要让他们的父盒子 `div#app` 高度设置为 `100%`。所以我们在 `src/App.vue` ：
...
### 然后配置 `src/components/login/login.vue` 组件样式
...
### 使用 Element 布局 Home 组件视图

### 使用 `Container 布局容器` 实现基本结构
